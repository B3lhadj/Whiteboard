...
export const getShapeSvg = (
  ...
  const markerId = `shape-marker-${shapeMarkerIdCounter++}`
  const arrowHeadId = `${markerId}-head`
  const arrowTailId = `${markerId}-tail`
  ...
  const marker = `
    <defs>
      <marker id="${arrowHeadId}" markerWidth="10" markerHeight="10" refX="8" refY="5" orient="auto">
        <path d="M 0 0 L 10 5 L 0 10 z" fill="${stroke}" />
      </marker>
      <marker id="${arrowTailId}" markerWidth="10" markerHeight="10" refX="2" refY="5" orient="auto">
  ...
  const content: Record<ShapeKind, string> = {
    ...
    arrow: `<line x1="12" y1="${height - 12}" x2="${width - 14}" y2="12" ${lineAttrs} marker-end="url(#${arrowHeadId})" />`,
    'arrow-down': `<line x1="12" y1="12" x2="${width - 14}" y2="${height - 12}" ${lineAttrs} marker-end="url(#${arrowHeadId})" />`,
    'double-arrow': `<line x1="14" y1="${height - 12}" x2="${width - 14}" y2="12" ${lineAttrs} marker-start="url(#${arrowTailId})" marker-end="url(#${arrowHeadId})" />`,
    'double-arrow-down': `<line x1="14" y1="12" x2="${width - 14}" y2="${height - 12}" ${lineAttrs} marker-start="url(#${arrowTailId})" marker-end="url(#${arrowHeadId})" />`,
    ...
    'elbow-arrow': `<polyline points="18,18 18,${height - 18} ${width - 18},${height - 18}" ${lineAttrs} marker-end="url(#${arrowHeadId})" />`,
    'left-arrow': `<line x1="${width - 14}" y1="${height - 12}" x2="14" y2="12" ${lineAttrs} marker-end="url(#${arrowHeadId})" />`,
